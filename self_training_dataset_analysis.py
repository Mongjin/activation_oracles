...
def construct_batch(
    ...
    for data_point in training_data:
        ...
        print(
            f"padding length: {padding_length}, len padding tokens: {len(padding_tokens)}"
        )
        if padding_length > 0:
            print(f"positions before padding: {positions}")
            positions = [p + padding_length for p in positions]
            print(f"positions after padding: {positions}")
...
cfg.training_data_filename = (
    "contrastive_rewriting_results_google_gemma-2-9b-it_num_features_2000.pkl"
)
...
# %%
t_batch = training_data[:8]
t_batch = construct_batch(t_batch, tokenizer, device)

# %%
print(t_batch["input_ids"][0].shape)
print(tokenizer.decode(t_batch["input_ids"][0]))
print(tokenizer.decode(t_batch["input_ids"][1]))

idx = 1

masked_tokens = t_batch["labels"][idx] == -100
print("masked tokens:")
print(tokenizer.decode(t_batch["input_ids"][idx][masked_tokens]))

print("non-masked tokens:")
print(tokenizer.decode(t_batch["input_ids"][idx][~masked_tokens]))
# %%
print(t_batch["positions"][idx])
print(tokenizer.decode(t_batch["input_ids"][idx][t_batch["positions"][idx]]))
# %%
print(t_batch["feature_indices"])

# %%
print((t_batch["steering_vectors"][0][0] - t_batch["steering_vectors"][1][0]).sum())
print((t_batch["steering_vectors"][0][0] - t_batch["steering_vectors"][-1][0]).sum())
# %%
