...
def get_selected_prompt_sets(results: dict) -> dict[str, set[str]]:
    selected_prompt_sets = {}
    for layer_percent, layer_info in results["selected_prompt_band"].items():
        selected_prompt_sets[layer_percent] = set(layer_info["summary"]["selected_prompt_texts"])
    return selected_prompt_sets


def classify_prompt_membership(prompt_text: str, selected_prompt_set: set[str]) -> str:
    if prompt_text in selected_prompt_set:
        return "selected_band"
    return "other"


def build_prompt_level_rows(results: dict, probe_metric: str) -> list[dict]:
    selected_prompt_sets = get_selected_prompt_sets(results)
    rows = []

    for source_name, layers_info in results["combined_prompt_rows"].items():
        for layer_percent, modes_info in layers_info.items():
            selected_prompt_set = selected_prompt_sets[layer_percent]
                    ...
                    rows.append(
                        {
                            ...
                            "mode": mode_name,
                            "group_membership": classify_prompt_membership(row["prompt_text"], selected_prompt_set),
...
def draw_grouped_delta_panel(
    ...
    group_order = ["selected_band", "other"]
    mode_order = ["add", "subtract"]
    x_positions = {
        ("add", "selected_band"): 0,
        ("add", "other"): 1,
        ("subtract", "selected_band"): 3,
        ("subtract", "other"): 4,
    }
...
def draw_source_delta_figure(
    ...
    fig.suptitle(f"Axis 2 intervention deltas by prompt group | source={source_name} | probe={probe_metric}")
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
def draw_gap_scatter(ax, rows: list[dict], mode_name: str, title: str) -> None:
    ...
    for group_name in ["selected_band", "other"]:
...
def draw_gap_figure(gap_rows: list[dict], output_path: Path, layer_percent: int) -> None:
    ...
    fig.suptitle(f"Axis 2 hider-guesser Oracle gap changes | Layer {layer_percent}%")
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    if args.output_dir is None:
        output_dir = Path(
            "./images/taboo_axis2_intervention",
            sanitize_name(input_json.parent.name),
        )
    else:
    ...
    save_csv(
        output_path=output_dir / "axis2_gap_rows.csv",
        rows=gap_rows,
        fieldnames=list(gap_rows[0].keys()),
    )
    save_csv(
        output_path=output_dir / "axis2_gap_summary.csv",
    ...
    for layer_percent in sorted({row["layer_percent"] for row in gap_rows}):
        draw_gap_figure(
            gap_rows=gap_rows,
            output_path=output_dir / f"axis2_gap_changes_layer_{layer_percent}.png",
            layer_percent=layer_percent,
        )

    for probe_metric in args.probe_metrics:
        if probe_metric not in available_metrics:
            raise ValueError(f"Probe metric '{probe_metric}' not found in Axis 2 prompt rows")
        ...
        save_csv(
            output_path=output_dir / f"axis2_prompt_rows_{metric_tag}.csv",
            rows=prompt_rows,
            fieldnames=list(prompt_rows[0].keys()),
        )
        save_csv(
            output_path=output_dir / f"axis2_prompt_summary_{metric_tag}.csv",
            rows=summary_rows,
        ...
        for source_name in ["hider", "guesser"]:
            draw_source_delta_figure(
                ...
                probe_metric=probe_metric,
                output_path=output_dir / f"axis2_deltas_{source_name}_{metric_tag}.png",
    ...
    summary_json = {
        ...
        "selected_prompt_band": results["selected_prompt_band"],
    }
    with open(output_dir / "axis2_plot_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary_json, f, indent=2)

    print(f"Saved Axis 2 plots to {output_dir}")
