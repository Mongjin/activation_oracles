...
def draw_source_delta_figure(
    ...
    fig.suptitle(
        f"Vanilla low-high contrast intervention deltas by prompt group | "
        f"source={source_name} | probe={probe_metric}"
    )
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
def draw_gap_figure(gap_rows: list[dict], output_path: Path, layer_percent: int) -> None:
    ...
    fig.suptitle(f"Vanilla low-high contrast | hider-guesser Oracle gap changes | Layer {layer_percent}%")
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    if args.output_dir is None:
        output_dir = Path(
            "./images/taboo_vanilla_low_high_contrast_intervention",
    ...
    save_csv(
        output_path=output_dir / "vanilla_low_high_gap_rows.csv",
        rows=gap_rows,
        fieldnames=list(gap_rows[0].keys()),
    )
    save_csv(
        output_path=output_dir / "vanilla_low_high_gap_summary.csv",
    ...
    for layer_percent in sorted({row["layer_percent"] for row in gap_rows}):
        draw_gap_figure(
            gap_rows=gap_rows,
            output_path=output_dir / f"vanilla_low_high_gap_changes_layer_{layer_percent}.png",
            layer_percent=layer_percent,
        )

    for probe_metric in args.probe_metrics:
        if probe_metric not in available_metrics:
            raise ValueError(f"Probe metric '{probe_metric}' not found in vanilla low-high prompt rows")
        ...
        save_csv(
            output_path=output_dir / f"vanilla_low_high_prompt_rows_{metric_tag}.csv",
            rows=prompt_rows,
            fieldnames=list(prompt_rows[0].keys()),
        )
        save_csv(
            output_path=output_dir / f"vanilla_low_high_prompt_summary_{metric_tag}.csv",
            rows=summary_rows,
        ...
        for source_name in ["hider", "guesser"]:
            draw_source_delta_figure(
                ...
                probe_metric=probe_metric,
                output_path=output_dir / f"vanilla_low_high_deltas_{source_name}_{metric_tag}.png",
    ...
    with open(output_dir / "vanilla_low_high_plot_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary_json, f, indent=2)

    print(f"Saved vanilla low-high contrast plots to {output_dir}")
